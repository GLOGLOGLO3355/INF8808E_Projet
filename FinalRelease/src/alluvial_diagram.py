...
def build_category_color_map(df, cols):
    ...
    extended_palette = (palette * ((len(all_categories) // len(palette)) + 1))[
        : len(all_categories)
    ]
...
def build_links(df, cols, labels, category_color_map):
    ...
    for i in range(len(cols) - 1):
        pair_counts = (
            df.groupby([cols[i], cols[i + 1]]).size().reset_index(name="count")
        )
...
def get_alluvial_diagram():
    ...
    df = df.dropna(
        subset=["Family_Income", "School_Type", "Teacher_Quality", "Exam_Score"]
    )
    ...
    source, target, values, link_colors = build_links(
        df, cols, labels, category_color_map
    )
